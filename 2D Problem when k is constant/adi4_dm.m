...
for i=1:N
    ii = Number(1,i);       B(ii) = (5*h*h*k/6-2)*uy1(y(i))...
                                    +(1+ k*h*h/12)*uy1(y(i)+h)...
                                    +(1+ k*h*h/12)*uy1(y(i)-h);
    ii = Number(N,i);       B(ii) = (5*h*h*k/6-2 )*uyend(y(i))...
                                    +(1+ k*h*h/12)*uyend(y(i)+h)...
                                    +(1+ k*h*h/12)*uyend(y(i)-h); %using given BC and next step2
end
...
for i=2:N-1
    for j=1:N
        ii = Number(i,j);
        A1(ii,ii) = 5*(k*h^2)/6 - 2;
        A1(ii,Number(i+1,j)) = 1+ (k*h^2)/12;      
        A1(ii,Number(i-1,j)) = 1+ (k*h^2)/12; 
    end
end
