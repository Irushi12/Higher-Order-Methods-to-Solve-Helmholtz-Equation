...
for i=2:N+1
    for j=1:N+2
        B1(i,j)= (2/15)*f2(i+1,j)/k+(11/15)*f2(i,j)/k+(2/15)*f2(i-1,j)/k;
    end
end
...
for i=1:N+2
    ii = Number(1,i);       
    B(ii) = (1-2*((1/(k*h^2)) + (2/15))+6*(1/(20*h^2*k)))*uy0(y(i))...
        +(((1/(k*h^2)) + (2/15)) ...
        - 4*(1/(20*h^2*k)))*uy0(y(i)+h)+(((1/(k*h^2)) + (2/15)) ...
        - 4*(1/(20*h^2*k)))*uy0(y(i)-h)...
        +(1/(20*h^2*k))*uy0(y(i)+2*h)...
        +(1/(20*h^2*k))*uy0(y(i)-2*h);
    ii = Number(N+2,i);     
    B(ii) = (1-2*((1/(k*h^2)) + (2/15))+6*(1/(20*h^2*k)))*uyend2(y(i))...
        +(((1/(k*h^2)) + (2/15)) ...
        - 4*(1/(20*h^2*k)))*uyend2(y(i)+h)+(((1/(k*h^2)) + (2/15)) ...
        - 4*(1/(20*h^2*k)))*uyend2(y(i)-h) ...
        +(1/(20*h^2*k))*uyend2(y(i)+2*h) ...
        +(1/(20*h^2*k))*uyend2(y(i)-2*h);
end
% solving for U_star
...
for i=3:N
    for j=3:N
        ii = Number(i,j);
        A1(ii,ii) = 1-2*((1/(k*h^2)) + (2/15))+6*(1/(20*h^2*k));
        A1(ii,Number(i+1,j)) = ((1/(k*h^2)) + (2/15)) - 4*(1/(20*h^2*k));      
        A1(ii,Number(i-1,j)) = ((1/(k*h^2)) + (2/15)) - 4*(1/(20*h^2*k));
        A1(ii,Number(i+2,j)) = (1/(20*h^2*k));      
        A1(ii,Number(i-2,j)) = (1/(20*h^2*k));
    end
end
U_star=A1\B';
...
for i=3:N
    for j=3:N
        ii = Number(i,j);
        A(ii,ii) = 1-2*((1/(k*h^2)) + (2/15))+6*(1/(20*h^2*k));
        A(ii,Number(i,j+1)) = ((1/(k*h^2)) + (2/15)) -4*(1/(20*h^2*k));      
        A(ii,Number(i,j-1)) = ((1/(k*h^2)) + (2/15)) -4*(1/(20*h^2*k));
        A(ii,Number(i,j+2)) = (1/(20*h^2*k));      
        A(ii,Number(i,j-2)) = (1/(20*h^2*k));
    end
end
B_col = U_star;
