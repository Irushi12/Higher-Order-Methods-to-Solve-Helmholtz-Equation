function [u] = cm_4_cg_2d(N,ux1,uxend,uy1,uyend,f,k)
...
for i=2:N-1
    for j=2:N-1
        ii = Number(i,j);
        A(ii,ii) = (25*h*h*k/36) -(10/3);
        A(ii,Number(i+1,j)) = (2/3) + (5*h*h*k/72); 
        A(ii,Number(i,j+1)) = (2/3) + (5*h*h*k/72); 
        A(ii,Number(i,j-1)) = (2/3) + (5*h*h*k/72);
        A(ii,Number(i-1,j)) = (2/3) + (5*h*h*k/72); 
        A(ii,Number(i+1,j+1)) =(1/6) + (h*h*k/144);
        A(ii,Number(i+1,j-1)) =(1/6) + (h*h*k/144); 
        A(ii,Number(i-1,j+1)) =(1/6) + (h*h*k/144);
        A(ii,Number(i-1,j-1)) =(1/6) + (h*h*k/144);
    end
